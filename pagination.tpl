...
{if isset($p) AND $p}
	...
	{if $start!=$stop}
		...
		{if $p != 1}
			...
			<li id="pagination_previous{if isset($paginationId)}_{$paginationId}{/if}" class="pagination_previous"><a {$no_follow_text} href="{$link->goPage($requestPage, $p_previous)}">&laquo;<span class="pagination-slova"> {l s='Předchozí'}</a></li>
		{else}
			<li id="pagination_previous{if isset($paginationId)}_{$paginationId}{/if}" class="disabled pagination_previous"><span>&laquo;<span class="pagination-slova"> {l s='Předchozí'}</span></li>
		{/if}
		{if $start==3}
		...
		{if $pages_nb > 1 AND $p != $pages_nb}
			...
			<li id="pagination_next{if isset($paginationId)}_{$paginationId}{/if}" class="pagination_next"><a {$no_follow_text} href="{$link->goPage($requestPage, $p_next)}"><span class="pagination-slova">{l s='Další'}</span> &raquo;</a></li>
		{else}
			<li id="pagination_next{if isset($paginationId)}_{$paginationId}{/if}" class="disabled pagination_next"><span><span class="pagination-slova">{l s='Další'}</span> &raquo;</span></li>
		{/if}
		</ul>
	{/if}
	{if $nb_products > $products_per_page}
		<form action="{if !is_array($requestNb)}{$requestNb}{else}{$requestNb.requestUrl}{/if}" method="get" class="form-inline pagination pull-right">
			...
			{if is_array($requestNb)}
				...
				{/foreach}
			{/if}
			<input type="submit" class="button_mini btn btn-sm" value="{l s='OK'}" />
		</form>
	{/if}
	</div>
	<!-- /Pagination -->
