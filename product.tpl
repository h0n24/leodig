...
<div id="primary_block" class="clearfix">
			...
			<span id="view_full_size">
				...
				<div class="img-product-tags">
						...
						{if $product->specificPrice.reduction_type == 'percentage'}
							<a href="http://www.helveti.cz/s/3/hodinky-akce"><span class="product_tag hot">{l s='Reduced price!'} -{$product->specificPrice.reduction*100}%</span></a><br />
						{/if}
						{if $product->specificPrice.reduction_type == 'amount'}
							<a href="http://www.helveti.cz/s/3/hodinky-akce"><span class="product_tag hot">{l s='Reduced price!'} -{convertPrice price=$productPriceWithoutReduction-$productPrice|floatval}</span></a><br />
						{/if}
						{if Product::jeNovinka($product->id)}<a href="http://www.helveti.cz/s/4/novinky"><span class="product_tag new">{l s='Novinka'}</span></a><br />{/if}
						{if $product->top200}
							<a href="http://www.helveti.cz/s/2/nejprodavanejsi-hodinky"><span class="product_tag nejprodavanejsi">{l s='Nejprodávanější'}</span></a><br />
						{/if}
						{if $product->condition=="used"}
							<a href="http://www.helveti.cz/s/5/bazar-hodinek"><span class="product_tag bazar">{l s='Bazar'}</span></a><br />
						{/if}
				</div>
				{*!custom nox*}
		...
		<div id="thumbs_list">
			<ul id="thumbs_list_frame-2">
				{if isset($images)}
					...
					{/if}
					<li id="thumbnail_{$image.id_image}" class="miniatura-img">
	...
	<div id="pb-left-column" class="col-sm-7 col-xs-12">
		...
		<div id="short_description_block">
			{if $product->description_short}
				<div id="short_description_content" class="rte align_justify short_description_content"><p class=""></p>{$product->description_short} <a href="javascript:{ldelim}{rdelim}" class="button vice">{l s='More details'}</a></div>
			{/if}
	...
	{assign var=pocet_hodnoceni value=Product::pocetHeurekaHvezd($product->id,'pocet_hodnoceni')} 
	{assign var=pocet_hvezd_na_desetinu value=Product::pocetHeurekaHvezd($product->id,'hvezdy_na_desetinu')} 
	{assign var=pocet_hvezd value=Product::pocetHeurekaHvezd($product->id,'hvezdy')} 	
		<div class="produkt-dalsi-info">
			<div class="box_prodejce"><a href="{$link->getmanufacturerLink($product->id_manufacturer)}" title="Autorizovaný prodejce značky {$product->manufacturer_name}"><img src="{$img_dir}helveti/autorizovany-prodejce-{$product->manufacturer_name|lower}.png" alt="Autorizovaný prodejce značky {$product->manufacturer_name}"></a></div>
			<div class="box_heureka"><img src="{$img_dir}/helveti/heureka-male-logo.png" alt="Heureka logo"><div class="box_heureka_txt"><span class="pocet_hodnoceni_heureka">{NoxHome::hodnoceniEshopu()}% </span><br><span class="pocet_hodnoceni_heureka_popis">Hodnocení</span><br /> <span class="pocet_hodnoceni_heureka_popis2">zákazníků</span></div></div>
			...
			({$pocet_hvezd_na_desetinu} / 5 bodů)<br />
			{$pocet_hodnoceni} uživatelů hodnotilo
			</div>
		</div>
		...
		{if $product->show_price AND !isset($restricted_country_mode) AND !$PS_CATALOG_MODE}
			...
			{if $priceDisplay >= 0 && $priceDisplay <= 2}
					...
					| <span id="usetrite">{l s='Ušetříte'} {convertPrice price=$productPriceWithoutReduction-$productPrice}</span>
			{/if}
			</span>
			</div>
		</div>
	{/if}
		...
		{if ($product->quantity <= 0 && !$product->available_later ) OR ($product->quantity > 0 && !$product->available_now) OR !$product->available_for_order OR $PS_CATALOG_MODE}
			{if $product->id_manufacturer==3||$product->id_manufacturer==8||$product->id_manufacturer==9}
				<p class="barva_neskladem nelze_koupit"><strong>Dostupné na dotaz.</strong></p>
				<p class="nelze_koupit_kontakt"> Pro více informací nás 	<a href="{$link->getCMSLink('8')}">kontaktujte</a>.</p>
			{else}
				<p class="barva_neskladem nelze_koupit"><strong>Hodinky nelze zakoupit.</strong></p>
				<p class="nelze_koupit_kontakt">Vyberte si prosím jiné nebo nás 	<a href="{$link->getCMSLink('8')}">kontaktujte</a>.</p>
			{/if} 
		{/if}
			...
			<a href="{$link->getCMSLink('39')}"><span class="produkt-iconky-pojisteni produkt-iconky-icon">
				Pojištění hodinek
			</span></a>
			<a href="{$link->getCMSLink('1')}"><span class="produkt-iconky-doprava produkt-iconky-icon">
				Doprava zdarma
			</span></a>
		...
		{foreach from=$features item=feature}
            {if isset($feature.value)}
            	{if !$feature.name|strstr:"FILTRACE"}
			    	<tr><td>{$feature.name|escape:'htmlall':'UTF-8'}</td><td> {$feature.value|escape:'htmlall':'UTF-8'}</td></tr>
			    {/if}
            {/if}
		{/foreach}
		</table>
		...
		<div id="idTab4" class="bullet row">
			<div class="products_block accessories_block clearfix">
				<div class="block_content">
					{assign var="acclimit" value=1}
					{assign var="columnpage" value=3}
								...
								<div class="product-container clearfix">
									<div class="center_block">									
										<a href="{$accessoryLink|escape:'htmlall':'UTF-8'}" title="{$accessory.legend|escape:'htmlall':'UTF-8'}" class="product_image">
											...
											<span class="product-additional" rel="{$accessory.id_product}"></span>
										</a>
									</div>
									<div class="right_block">
										...
										{if !$PS_CATALOG_MODE && ($accessory.allow_oosp || $accessory.quantity > 0)}
											<a class="button ajax_add_to_cart_button exclusive pull-left" href="{$link->getPageLink('cart', true, NULL, "qty=1&amp;id_product={$accessory.id_product|intval}&amp;token={$static_token}&amp;add")|escape:'html'}" rel="ajax_id_product_{$accessory.id_product|intval}" title="{l s='Add to cart'}">{l s='Add to cart'}</a>
										{/if}
										      <a  href="#" id="wishlist_button{$accessory.id_product}" class="btn-tooltip button wishlist-compare fa fa-heart" onclick="LeoWishlistCart('wishlist_block_list', 'add', '{$accessory.id_product}', $('#idCombination').val(), 1 ); return false;" title="{l s='Add to wishlist'}"></a>
										      <a class="quick-view btn-tooltip button wishlist-compare fa fa-arrows-alt" title="{l s='Quick View'}" href="{if $accessory.link|strpos:"?"}{$accessory.link|cat:"&content_only=1"|escape:'htmlall':'UTF-8'}{else}{$accessory.link|cat:"?content_only=1"|escape:'htmlall':'UTF-8'}{/if}"></a>
									</div>
								</div>
							</div>
							{if $acclimit % $columnpage==0 || $smarty.foreach.accessories_list.last}
								</div>
							{/if}
								{$acclimit = $acclimit + 1 }
						{/if}						
