...
<div id="primary_block" class="block clearfix">
	...
	<div id="pb-right-column" class="col-sm-5 col-xs-12">
		...
		{if $have_image}
			<span id="view_full_size">
				...
				{*custom nox*}
						{if isset($product->new) && $product->new == 1}<span class="new">{l s='Novinka'}</span>{/if}
						{if $product->specificPrice.reduction_type == 'percentage'}
							<span class="hot">-{$product->specificPrice.reduction*100}%</span>
						{/if}
						{if $product->specificPrice.reduction_type == 'amount'}
							<span class="hot">-{convertPrice price=$productPriceWithoutReduction-$productPrice|floatval}</span>
						{/if}
		...
		{if isset($images) && count($images) > 3}<span class="view_scroll_spacer"><a id="view_scroll_left" class="hidden" title="{l s='Other views'}" href="javascript:{ldelim}{rdelim}"><i class="fa fa-chevron-circle-left"></i></a></span>{/if}
		<div id="thumbs_list">
	...
	<div id="pb-left-column" class="col-sm-7 col-xs-12">
		...
		<div class="content_prices clearfix kosik_box" style="border:1px solid #666; padding:8px;background:#ddd">
			...
			<p class="doprava_zdarma_box">Doprava Zdarma</p>
			<p class="heureka_hodnoceni_box">ZDE BUDE HODNOCENI ESHOPU</p>
			</div>
