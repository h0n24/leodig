...
{if !$opc}
	...
	var orderUrl = '{$link->getPageLink("order", true)|addslashes}';s
...
{if !$opc}
	<h1>{l s='Choose your delivery method'}</h1>
{else}
	...
	{if isset($delivery_option_list)}
		{foreach $delivery_option_list as $id_address => $option_list}
			...
			{foreach $option_list as $key => $option}
				<div class="delivery_option {if ($option@index % 2)}alternate_{/if}item form-group clearfix">
						...
						<table class="resume col-sm-12">
							<tr>
								...
								<td class="delivery_option_icko">
									{assign var='dodani' value=Carrier::dodaniDoprava($cart->id,$carrier.instance->name)}
									{if $dodani}
										{$dodani}
									{else}
								...
								<td class="delivery_option_price">
									{if $option.total_price_with_tax && (!isset($free_shipping) || (isset($free_shipping) && !$free_shipping))}
										...
										{/if}
									{else}
										<div class="free-shipping">{l s='Free'}</div>
									{/if}
								</td>
								<td class="delivery_option_icko">
									<span class="btn-tooltip doprava_ikonka_i ikonka_i" data-html="true" data-placement="right" data-original-title="{Carrier::informaceDoprava({$carrier.instance->name})}">i</span>
								</td>
							</tr>
						</table>
						<table class="col-sm-12 delivery_option_carrier {if isset($delivery_option[$id_address]) && $delivery_option[$id_address] == $key}selected{/if} {if $option.unique_carrier}not-displayable{/if}">
							{foreach $option.carrier_list as $carrier}
							<tr>
			...
			{**INPUT PRO ULOZENKU**}
			<input type="hidden" id="ulozenka_id_custom" value="{Configuration::get('ULOZENKA_CARRIER_ID')}"/>
...
{if !$opc}
	<p class="cart_navigation submit">
		...
		{if !$is_guest}
			{if $back}
				<a href="{$link->getPageLink('order', true, NULL, "step=1&back={$back}&multi-shipping={$multi_shipping}")|escape:'html'}" title="{l s='Previous'}" class="zpet-do-obchodu"><i class="fa fa-chevron-left"></i> {l s='Zpět na dodací údaje'}</a>
			{else}
				<a href="{$link->getPageLink('order', true, NULL, "step=1&multi-shipping={$multi_shipping}")|escape:'html'}" title="{l s='Previous'}" class="zpet-do-obchodu"><i class="fa fa-chevron-left"></i> {l s='Zpět na dodací údaje'}</a>
			{/if}
		{else}
				<a href="{$link->getPageLink('order', true, NULL, "multi-shipping={$multi_shipping}")|escape:'html'}" title="{l s='Previous'}" class="zpet-do-obchodu"><i class="fa fa-chevron-left"></i> {l s='Zpět na dodací údaje'}</a>
		{/if}
		{if isset($virtual_cart) && $virtual_cart || (isset($delivery_option_list) && !empty($delivery_option_list))}
		
			<input type="submit" name="processCarrier" value="{l s='Vyberte způsob platby'}" class="standard-checkout pull-right" />
		{/if}
	</p>
</form>
{else}
