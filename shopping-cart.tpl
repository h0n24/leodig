	...
	<table id="cart_summary" class="std table-bordered">
		<thead>
			<tr>
				...
				<th class="cart_ref item">{l s='Dostupnost'}</th>
				<th class="cart_unit item">{l s='Cena za kus'}</th>
				<th class="cart_quantity item">{l s='Množství'}</th>
			...
			<tr class="cart_total_price">
				<td colspan="4" id="cart_voucher" class="cart_voucher">
					<div id="kosik_dopravy" class="kosik_moznosti">
						...
						<div id="ceskaposta" class="ikonka_moznosti"><img src="{$img_dir}/helveti/kosik-c-posta.png" alt="Česká pošta" title="Česká pošta"></div>
						<div id="dpd" class="ikonka_moznosti"><img src="{$img_dir}/helveti/kosik-dpd.png" alt="DPD" title="DPD"></div>
						<div id="ppl" class="ikonka_moznosti"><img src="{$img_dir}/helveti/kosik-ppl.png" alt="PPL" title="PPL"></div>
						<div id="ulozenka" class="ikonka_moznosti"><img src="{$img_dir}/helveti/kosik-ulozenka.png" alt="Uloženka" title="Uloženka"></div>
						<div id="osobni" class="ikonka_moznosti"><img src="{$img_dir}/helveti/kosik-osobne.png" alt="Osobní odběr" title="Osobní odběr"></div>
					</div>
					<div id="kosik_platby" class="kosik_moznosti">
						<div>Možnosti platby</div>
						<div id="hotovost" class="ikonka_moznosti"><img src="{$img_dir}/helveti/kosik-hotove.png" alt="Hotově" title="Hotově"></div>
						<div id="kartou" class="ikonka_moznosti"><img src="{$img_dir}/helveti/kosik-kartou.png" alt="Kartou" title="Kartou"></div>
						<div id="prevodem" class="ikonka_moznosti"><img src="{$img_dir}/helveti/kosik-prevodem.png" alt="Převodem" title="Převodem"></div>
					</div>
				...
				{if $voucherAllowed}
					...
					<form action="{if $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" id="voucher" class="form-inline">
						<fieldset {if !(isset($discount_name) && $discount_name)}style="display:none"{/if}>
						...
						<div class="form-group">
							<p class="submit"><input type="hidden" name="submitDiscount" /><input type="submit" name="submitAddDiscount" value="{l s='Vložit'}" class="btn button" /></p>
						</div>
						</fieldset>
					</form>
					{if $displayVouchers}
				...
				<td colspan="3" class="price total_price_container" id="total_price_container">
					...
					{if $shippingCost>0}
					<div id="kosik_box_doprava">Doprava: {displayPrice price=$shippingCost}</div>
					{else}
					<div id="kosik_box_doprava_zdarma"><img src="{$img_dir}/helveti/cart-car.png" class="cart-car"> Doprava zdarma</div>
					{/if}
					<p class="celkem_kosik">celkem: <span class="cena_celkem">{displayPrice price=$total_price}</span></p>
				</td>
				{else}
				<td colspan="3" class="price total_price_container" id="total_price_container">
					<div id="ceny_s_dph">ceny jsou uvedeny včetně DPH</div>
					<div id="kosik_box_doprava_zdarma"><img src="{$img_dir}/helveti/cart-car.png" class="cart-car"> Doprava zdarma</div>
					<p class="celkem_kosik">celkem: <span class="cena_celkem">{displayPrice price=$total_price_without_tax}</span></p>
				</td>
				{/if}
			</tr>
		</tfoot>
	{if sizeof($discounts)}
		...
		{foreach $discounts as $discount}
			<tr class="cart_discount {if $discount@last}last_item{elseif $discount@first}first_item{else}item{/if}" id="cart_discount_{$discount.id_discount}">
				...
				<td class="cart_discount_name cart_description" colspan="2"><p class="s_title_block">{$discount.name}</p></td>
				...
				<td class="price_discount_del cart_delete">
					{if strlen($discount.code)}<a href="{if $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}?deleteDiscount={$discount.id_discount}" class="price_discount_delete" title="{l s='Delete'}"><i class="fa fa-times" alt="{l s='Delete'}"></i></a>{/if}
				</td>
			</tr>
		{/foreach}
		</tbody>
...
<p class="cart_navigation">
	{if !$opc}
		<a href="{if $back}{$link->getPageLink('order', true, NULL, 'step=1&amp;back={$back}')}{else}{$link->getPageLink('order', true, NULL, 'step=1')}{/if}" class="button pull-right standard-checkout" title="{l s='Pokračovat v objednávce'}">{l s='Pokračovat v objednávce'} <i class="fa fa-chevron-right"></i></a>
		{if Configuration::get('PS_ALLOW_MULTISHIPPING')}
			<a href="{if $back}{$link->getPageLink('order', true, NULL, 'step=1&amp;back={$back}')}{else}{$link->getPageLink('order', true, NULL, 'step=1')}{/if}&amp;multi-shipping=1" class="pull-right multishipping-button multishipping-checkout button" title="{l s='Pokračovat v objednávce'}">{l s='Pokračovat v objednávce'} <i class="fa fa-chevron-right"></i></a>
		{/if}
	{/if}
