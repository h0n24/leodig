...
{if $products}
	...
	<div class="wishlistLinkTop">
		...
		<div class="wishlisturl form-horizontal clearfix">
			<label class="col-sm-2 control-label">{l s='Permalink' mod='blockwishlist'}:</label>
			<div class="col-sm-10">
				<input class="form-control" type="text" value="{$base_dir_ssl}modules/blockwishlist/view.php?token={$token_wish|escape:'htmlall':'UTF-8'}" readonly="readonly" />
			</div>
		</div>
		<p class="submit">
	...
	<div class="wlp_bought">
		...
		{foreach from=$products item=product name=i}
			<li id="wlp_{$product.id_product}_{$product.id_product_attribute}" class="clearfix address col-sm-4 col-xs-12 product_block {if ($smarty.foreach.i.index + 1) % 3 == 1}clearfix{/if}">
				<div class="product-container clearfix">
					...
					<div class="clearfix">
						<div class="product_image">
							<a href="{$link->getProductlink($product.id_product, $product.link_rewrite, $product.category_rewrite)|escape:'html'}" title="{l s='Product detail' mod='blockwishlist'}">
								<img class="img-responsive" src="{$link->getImageLink($product.link_rewrite, $product.cover, 'home_default')|escape:'html'}" alt="{$product.name|escape:'htmlall':'UTF-8'}" />
							</a>
						</div>
						<div class="product_infos">
							<h4 id="s_title" class="name">{$product.name|truncate:30:'...'|escape:'htmlall':'UTF-8'}</h4>
							...
							{/if}
								<br />
								<label for="quantity_{$product.id_product}_{$product.id_product_attribute}">{l s='Quantity' mod='blockwishlist'}:</label><input type="text" id="quantity_{$product.id_product}_{$product.id_product_attribute}" value="{$product.quantity|intval}" size="3"  />
								<br /><br />
					...
					<div class="btn_action">
						<a href="javascript:;" class="lnksave" onclick="WishlistProductManage('wlp_bought_{$product.id_product_attribute}', 'update', '{$id_wishlist}', '{$product.id_product}', '{$product.id_product_attribute}', $('#quantity_{$product.id_product}_{$product.id_product_attribute}').val(), $('#priority_{$product.id_product}_{$product.id_product_attribute}').val());" title="{l s='Save' mod='blockwishlist'}">{l s='Save' mod='blockwishlist'}</a>
					</div>
				</div>
			</li>
		{/foreach}
	...
	<form method="post" class="wl_send std hidden form-horizontal" onsubmit="return (false);">
		<fieldset>
			<div class="form-group required">
				<label class="col-sm-3 control-label" for="email1">{l s='Email' mod='blockwishlist'}1 <sup>*</sup></label>
				<div class="col-sm-9">
			...
			<div class="form-group">
				<label class="control-label col-sm-3" for="email{$smarty.section.i.index}">{l s='Email' mod='blockwishlist'}{$smarty.section.i.index}</label>
				<div class="col-sm-9">
					<input class="form-control" type="text" name="email{$smarty.section.i.index}" id="email{$smarty.section.i.index}" />
				</div>
			</div>
			{/section}
			<div class="form-group col-xs-12 submit" style="margin-top: 20px">
				<input class="button pull-left" type="submit" value="{l s='Send' mod='blockwishlist'}" name="submitWishlist" onclick="WishlistSend('wl_send', '{$id_wishlist}', 'email');" />
				<div class="pull-right required">
					<sup>*</sup> {l s='Povinná pole'}
				</div>
			</div>
		</fieldset>
	</form>
