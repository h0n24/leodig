...
class AdminOrdersControllerCore extends AdminController
{
    ...
    /**NOX - products**/
        ...
        IF((SELECT COUNT(do.id_order) FROM `'._DB_PREFIX_.'order_detail` do WHERE do.id_order = a.id_order) > 1, 
            CONCAT( \' (+\',
                ((SELECT COUNT(do.id_order) FROM `'._DB_PREFIX_.'order_detail` do WHERE do.id_order = a.id_order)-1),\' další)\')
            ,\'\')
            ) AS `products`,
		osl.`name` AS `osname`,
		os.`color`,
        ...
        $this->_group .= 'GROUP BY id_pdf';                 /**NOX**/
		...
		/**NOX**/
		'products' => array(
            'title' => $this->l('Zboží v objednávce'),
            'havingFilter' => true,
        ),
		...
		if (Tools::isSubmit('submitShippingNumber') && isset($order))
		{
			if ($this->tabAccess['edit'] === '1')
			{
				...
				else
				{
					...
					if($stav_objednavky!=4)$order->setCurrentState(4);
